...
class R_robot:
    ...
    MAX_INTIAL_PHI_TIME = 1000   #最大尝试次数
    ...
    def is_right_solution(self, x, y, z, theta1, theta2, theta3):
        '''通过正向运动学，确认逆向运动学的解是否正确'''
        nx = cos(theta1)*(self.L2*cos(theta2)+self.L3*cos(theta2+theta3))
        ny = sin(theta1)*(self.L2*cos(theta2)+self.L3*cos(theta2+theta3))
        nz = -(self.L2*sin(theta2)+self.L3*sin(theta2+theta3))
    ...
    def inverse_kinematics(self, wx, wy, wz):
        ...
        if wx == 0 and wy == 0:
            ...
            candi_joints.append([None, theta1, None, None])
        else:
        ...
        min_radius = min_radius2 + min_radius3
        max_radius = max_radius2 + max_radius3
        tmp_candi_joints = []
        for initial_num in range(self.MAX_INTIAL_PHI_TIME):
            ...
            for candi in candi_joints:
                theta1 = candi[1]
                C2 = wx/(cos(theta1)*self.L2) - self.L3*cos(phi)/self.L2
                S2 = -(wz+self.L3*sin(phi))/self.L2
                theta2 = math.atan2(S2, C2)
                theta3 = phi - theta2
                tmp_candi_joints.append([None, theta1, theta2, theta3])
            tmp_candi_joints = list(filter(self.is_legal_joint_radius, tmp_candi_joints))

            #根据正向运动学再次确认解是否正确
            tmp_candi_joints = list(filter(lambda candi: self.is_right_solution(wx, wy, wz, candi[1], candi[2], candi[3]), tmp_candi_joints))
            ...
            if len(tmp_candi_joints) == 0:
                ...
                for i_candi in range(len(tmp_candi_joints)):
                    t_theta2 = tmp_candi_joints[i_candi][2]
                    t_theta3 = tmp_candi_joints[i_candi][3]
                    theta4 = -t_theta2 - (math.pi/2) - t_theta3
                    tmp_candi_joints[i_candi].append(theta4)
...
if __name__ == "__main__":
    ...
    # for i in numpy.arange(0.01,0.10,0.01):
    #     ret, reu = arm.inverse_kinematics(0,0,i)
    #     if ret:
    #         print('{}xxxxxxxxx{}xxxxxxxxx{}'.format(ret, reu, i))

    ret, reu = arm.inverse_kinematics(0.01,0.03,0.04)
    if ret:
        print('{}'.format(reu))
        for i in range(len(reu)):
            print('t2: {} ; t3: {} ; t4: {}'.format(math.degrees(reu[i][2]), math.degrees(reu[i][3]), math.degrees(reu[i][4])))